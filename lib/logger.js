...
const logger = {
  ...
  userAction: (action, userId, userEmail, meta = {}) => {
    const message = meta.message || `User action: ${action}`;
    const log = formatLog('USER_ACTION', action, message, {
      userId,
      userEmail,
      ...meta
    });
...
function getRecentLogs(type = 'all', limit = 100, search = null, skip = 0) {
  ...
  if (search) {
    ...
    matchingLines = lines.filter(line => line.toLowerCase().includes(searchLower));
  }

  // Calculate slice indices for pagination (newest first)
  const total = matchingLines.length;
  ...
  const logs = matchingLines.slice(startIndex, endIndex).map(line => {
    try {
